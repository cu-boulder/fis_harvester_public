...
harvester-sparqltranslator -X subjectArea.config.xml

# Backup constructed data
harvester-transfer -i ../vivo.model.xml -ImodelName=http://vitro.mannlib.cornell.edu/a/graph/ws_sa_construct -d data/constructed-data

echo "Backing up data in ../backups/subjectArea-data-$DATE"
cd ..
if [ ! -d backups ]; then
  mkdir backups
fi
cd backups
touch subjectArea-data-$DATE
touch subjectArea-data-last
cd ../subjectAreas
