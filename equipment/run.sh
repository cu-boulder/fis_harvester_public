...
export HARVEST_NAME=dat-equipment
...
harvester-csvfetch -X csvfetch.config.xml >/dev/null

# Remove the original construct Model
harvester-jenaconnect -X delete.config.xml > /dev/null

# Execute Translate using SPARQL
# Modify our imported data using SPARQL into a new construct model
harvester-sparqltranslator -X equipment.config.xml >/dev/null

# Backup constructed data
harvester-transfer -i ../vivo.model.xml -ImodelName=http://vitro.mannlib.cornell.edu/a/graph/ws_eq_construct -d data/constructed-data >/dev/null

echo "Backing up data in ../backups/equipment-data-$DATE"
cd ..
if [ ! -d backups ]; then
  mkdir backups
fi
cd backups
touch equipment-data-$DATE
touch equipment-data-last
cd ../equipment

cp data/constructed-data ../backups/equipment-data-$DATE
cp data/constructed-data ../backups/equipment-data-last


#Output some counts
EQUIP=`cat data/constructed-data | grep 'Equipment' | wc -l`
echo "Imported $EQUIP pieces of equipment"
