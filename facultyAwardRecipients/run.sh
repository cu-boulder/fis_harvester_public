...
harvester-sparqltranslator -X awardrecipient.config.xml

# Backup constructed data
harvester-transfer -i ../vivo.model.xml -ImodelName=http://vitro.mannlib.cornell.edu/a/graph/ws_awr_construct -d data/constructed-data

echo "Backing up data in ../backups/faculty-data-$DATE"
cd ..
if [ ! -d backups ]; then
  mkdir backups
fi
cd backups
touch faculty-data-$DATE
touch faculty-data-last
cd ../faculty
...
#Output some counts
ORGS=`cat data/constructed-data | grep 'http://xmlns.com/foaf/0.1/Person' | wc -l`
echo "Imported $ORGS award orgs"
