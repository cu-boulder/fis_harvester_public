...
export HARVEST_NAME=dat-faculty-degrees
...
harvester-transfer -i ../vivo.model.xml -ImodelName=http://vitro.mannlib.cornell.edu/a/graph/ws_fm2dg_construct -d data/constructed-data

# Backup constructed data
echo "Backing up data in ../backups/facultyDegrees-data-$DATE"
cd ..
if [ ! -d backups ]; then
  mkdir backups
fi
cd backups
touch facultyDegrees-data-$DATE
touch facultyDegrees-data-last
cd ../facultyDegrees
...
#Output some counts
ACDEG=`cat data/constructed-data | grep '<rdf:type rdf:resource="http://vivoweb.org/ontology/core#EducationalTraining' | wc -l`

echo "Imported $ACDEG Academic Degrees"
