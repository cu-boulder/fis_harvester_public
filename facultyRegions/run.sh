...
harvester-transfer -i ../vivo.model.xml -ImodelName=http://vitro.mannlib.cornell.edu/a/graph/ws_fm2r_construct -d data/constructed-data

#Save a copy to backups
echo "Backing up data in ../backups/facultyRegions-data-$DATE"
cd ..
if [ ! -d backups ]; then
  mkdir backups
fi
cd backups
touch facultyRegions-data-$DATE
touch facultyRegions-data-last
cd ../facultyRegions

cp data/constructed-data ../backups/facultyRegions-data-$DATE
cp data/constructed-data ../backups/facultyRegions-data-last

#Output some counts

FAC=`cat data/constructed-data | grep '<rdf:Description rdf:about="https://experts.colorado.edu/individual/fisid_' | wc -l`
