...
harvester-transfer -i ../vivo.model.xml -ImodelName=http://vitro.mannlib.cornell.edu/a/graph/ws_fm2sa_construct -d data/constructed-data

# Backup the copy to a safe location (data is rebooted every ingest)
echo "Backing up data in ../backups/facultySubjectAreas-data-$DATE"
cd ..
if [ ! -d backups ]; then
  mkdir backups
fi
cd backups
touch facultySubjectAreas-data-$DATE
touch facultySubjectAreas-data-last
cd ../facultySubjectAreas
