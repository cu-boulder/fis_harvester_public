...
export HARVEST_NAME=dat-faculty-urls
...
harvester-sparqltranslator -X facultyURLs.config.xml

# Generate URI's for blank nodes
harvester-renameblanknodes -X rename.config.xml

# Save a copy of the model for restoration
harvester-transfer -i ../vivo.model.xml -ImodelName=http://vitro.mannlib.cornell.edu/a/graph/ws_fm2url_construct -d data/constructed-data

# Backup constructed data
echo "Backing up data in ../backups/facultyURLs-data-$DATE"
cd ..
if [ ! -d backups ]; then
  mkdir backups
fi
cd backups
touch facultyURLs-data-$DATE
touch facultyURLs-data-last
cd ../facultyUrls

cp data/constructed-data ../backups/facultyURLs-data-$DATE
cp data/constructed-data ../backups/facultyURLs-data-last


#Output some counts
URL=`cat data/constructed-data | grep '<rdf:type rdf:resource="http://vivoweb.org/ontology/core#URLLink' | wc -l`

echo "Imported $URL Webpages"
