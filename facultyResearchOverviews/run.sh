...
# Remove the original import Model
harvester-jenaconnect -X preRunDelete.config.xml
...
harvester-transfer -i ../vivo.model.xml -ImodelName=http://vitro.mannlib.cornell.edu/a/graph/ws_fm2ro_construct -d data/constructed-data

# Backup the copy to a safe location (data is rebooted every ingest)
echo "Backing up data in ../backups/facultyResearchOverviews-data-$DATE"
cd ..
if [ ! -d backups ]; then
  mkdir backups
fi
cd backups
touch facultyResearchOverviews-data-$DATE
touch facultyResearchOverviews-data-last
cd ../facultyResearchOverviews

cp data/constructed-data ../backups/facultyResearchOverviews-data-$DATE
cp data/constructed-data ../backups/facultyResearchOverviews-data-last
