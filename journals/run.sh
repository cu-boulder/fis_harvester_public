...
export HARVEST_NAME=dat-journals
...
harvester-sparqltranslator -X journals.config.xml
...
harvester-transfer -i ../vivo.model.xml -ImodelName=http://vitro.mannlib.cornell.edu/a/graph/ws_jour_construct -d data/constructed-data

#Save a copy to backups
echo "Backing up data in ../backups/journals-data-$DATE"
cd ..
if [ ! -d backups ]; then
  mkdir backups
fi
cd backups
touch journals-data-$DATE
touch journals-data-last
cd ../journals

cp data/constructed-data ../backups/journals-data-$DATE
cp data/constructed-data ../backups/journals-data-last

#Output some counts

JOUR=`cat data/constructed-data | grep '<rdf:Description rdf:about="https://experts.colorado.edu/individual/journal_' | wc -l`
REGIONS=`cat data/constructed-data | grep 'geographicFocus rdf:resource="http://aims.fao.org/aos/geopolitical.owl#' | wc -l`


echo "Imported $JOUR JOurnalss"
