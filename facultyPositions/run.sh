...
harvester-transfer -i ../vivo.model.xml -ImodelName=http://vitro.mannlib.cornell.edu/a/graph/ws_fm2p_construct -d data/constructed-data

#Save a copy to backups
echo "Backing up data in ../backups/facultyPositions-data-$DATE"
cd ..
if [ ! -d backups ]; then
  mkdir backups
fi
cd backups
touch facultyPositions-data-$DATE
touch facultyPositions-data-last
cd ../facultyPositions

cp data/constructed-data ../backups/facultyPositions-data-$DATE
cp data/constructed-data ../backups/facultyPositions-data-last
...
echo "Imported $POS Positions for $FAC Faculty Members"
