ALTER SESSION SET CURRENT_SCHEMA=Faculty_UCCS;
...
begin
  ...
  */
  ...
  l_cursor := vivo.getFacultyMemberHomeDepts();
  l_rows := unloader.run(p_cursor => l_cursor,
                         p_tname => NULL,
                         p_terminator => NULL,
                         p_dir => '/tmp',
                         p_filename => 'fis_faculty_member_home_depts',
                         p_ctl => 'NO',
                         p_header => 'YES');
  dbms_output.put_line(to_char(l_rows) || ' faculty member home departments extracted');
