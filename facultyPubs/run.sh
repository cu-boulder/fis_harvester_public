...
echo "Running csvfetch"
harvester-csvfetch -X csvfetch.config.xml

echo "Removing original model"
...
harvester-sparqltranslator -X pubs.config.xml

# Backup constructed data
harvester-transfer -i ../vivo.model.xml -ImodelName=http://vitro.mannlib.cornell.edu/a/graph/ws_fpub_construct -d data/constructed-data

echo "Backing up data in ../backups/works-data-$DATE"
cd ..
if [ ! -d backups ]; then
  mkdir backups
fi
cd backups
touch works-data-$DATE
touch works-data-last
cd ../facultyPubs

cp data/constructed-data ../backups/pubss-data-$DATE
cp data/constructed-data ../backups/pubs-data-last


#Output some counts
PUBS=`cat data/constructed-data | wc -l`
echo "Imported $PUBS works"
