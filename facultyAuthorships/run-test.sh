...
#DRE harvester-csvfetch -X csvfetch.config.xml
...
#DRE harvester-sparqltranslator -X auth.config.xml

# Backup constructed data
harvester-transfer -i ../vivo.model.xml -ImodelName=http://vitro.mannlib.cornell.edu/a/graph/ws_fauth_construct -d data/constructed-data

echo "Backing up data in ../backups/auth-data-$DATE"
cd ..
if [ ! -d backups ]; then
  mkdir backups
fi
cd backups
touch auth-data-$DATE
touch auth-data-last
cd ../facultyWorks

cp data/constructed-data ../backups/auth-data-$DATE
cp data/constructed-data ../backups/auth-data-last


#Output some counts
AUTHS=`cat data/constructed-data | wc -l`
echo "Imported $AUTHS authorships"
